...
void stepSequencer::setup(DistControl * Scale,int * Transpose, int NumberOfNotes, control * Tempo, int Dwell, float R, float G, float B, int * SampleRate,int * Chord, ofxMidiOut * midinote,bool*Conf){
    ...
    noteFrames=tempo->value/60.0f*FRAMERATE;
    ...
    for(int i=0;i<maxNotes;i++){
        ...
        rownote[i]=scale[i%7].value;
        rowoct[i]=(int)i/7+octave.value;
...
//    }
	for(int i=0;i<numberOfNotes.value;i++)
            for(int j=0;j<numberOfNotes.value;j++){
				if(seqNote[i][j].sounds){
					...
					//seqNote[i][j].StepMidiOut->sendNoteOff(MIDICH2, seqNote[i][j].midinote, 0);
				}
			}
    ...
    prChord=*chord;
    for(int i=0;i<maxNotes;i++){
        rownote[i]=scale[(i+*chord)%7].value;
        rowoct[i]=(int)(i+*chord)/7+octave.value+localOctave;
...
void stepSequencer::draw(){
	...
	ofDrawBitmapString(temp,ofGetWidth()/2,ofGetHeight()/2);*/
}
