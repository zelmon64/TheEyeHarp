...
void DistControl::setup(char Name[20] ,ofPoint posup, ofPoint posdw,
    ...
    step=Step;
    value=setval;
	dwell=(int)(FRAMERATE*(float)Dwell/1000.0); //
    ...
    namePos.x=(posUP.x+posDW.x)/2-20;
...
void  DistControl::draw(){
    ...
    ofDrawBitmapString(name,namePos.x ,namePos.y);
    ofDrawBitmapString(ofToString(value),namePos.x ,namePos.y+15);
	if(activeUP)
		ofDrawBitmapString(ofToString(value),magPosUP.x ,magPosUP.y+15);
	if(activeDW)
		ofDrawBitmapString(ofToString(value),magPosDW.x ,magPosDW.y+15);
}
