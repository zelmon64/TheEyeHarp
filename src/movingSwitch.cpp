...
void movingSwitch::update(ofPoint Gaze){
    ...
    if(ofDist(gaze.x,gaze.y,pos.x,pos.y)<size || value){
        ...
        if(value){
            ...
            if(ofDist(gaze.x,gaze.y,prGaze.x,prGaze.y)*FRAMERATE < 500) {//if fixation detected
                if(FC++ >= dwell){
                    value=false;
...
void movingSwitch::draw(string xval,string yval){
    ...
    string x="";
    string y="";
    ofDrawBitmapString(x.append(xval),pos.x ,pos.y-20);
    ofDrawBitmapString(y.append(yval),pos.x ,pos.y+25);
